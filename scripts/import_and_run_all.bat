...
"%PYTHON_EXE%" %PYTHON_ARGS% "scripts\run_daily_update.py"
if errorlevel 1 (
  echo.
...
echo.
echo 3. Excel 报告：
"%PYTHON_EXE%" %PYTHON_ARGS% -c "import pathlib; out=pathlib.Path(r'%PROJECT_DIR%')/'data'/'output'; p=max(out.glob('amazon_ops_report_*.xlsx'), key=lambda x: x.stat().st_mtime, default=out/'amazon_ops_report_YYYY-MM-DD.xlsx'); print(p)"
echo.
echo 4. 导入日志：
