...
def _render_action_effect_review_rows(rows: list[dict[str, str]], limit: int = 5) -> str:
    ...
    rank = {"变差": 0, "暂未改善": 1, "无明显变化": 2, "待7天确认": 3, "样本不足": 4, "有改善迹象": 5, "初步有效": 6}
    sorted_rows = sorted(rows, key=lambda row: (rank.get(str(row.get("judgement") or ""), 9), str(row.get("marketplace") or "")))[:limit]
    parts = ['<div class="review-list">']
    for row in sorted_rows:
        judgement = str(row.get("judgement") or "数据不足")
        tag = "tag-green" if judgement in {"初步有效", "有改善迹象", "有效"} else ("tag-yellow" if judgement in {"样本不足", "待7天确认", "数据不足"} else "tag-red")
        title = f"{row.get('marketplace') or ''}｜{row.get('product_name') or ''}"
        parts.append(
            "\n".join(
                [
                    ...
                    f'<div class="search-term-meta">下一步：{_inline_markup(str(row.get("next_step") or ""))}</div>',
                    "</div>",
                ]
            )
        )
...
def _keyword_review_sort_key(row: dict[str, str]) -> tuple[int, int, str, str]:
    ...
    days_text = str(row.get("days_since_execution") or "")
    try:
        days = int(float(days_text))
    except ValueError:
        days = -1
    return (
        priority.get(str(row.get("judgement") or ""), 9),
        -days,
...
def _keyword_review_attribution_label(row: dict[str, str]) -> str:
    ...
    if _is_truthy_flag(row.get("promoted_conversion_improved")):
        label = "本 SKU 成交有效"
    elif _is_truthy_flag(row.get("halo_only_conversion")):
...
def _keyword_review_metric_row(row: dict[str, str], prefix: str, label: str) -> str:
    metrics = [
        f'{label} 点击 {row.get(f"{prefix}_clicks")}',
        f'花费 {row.get(f"{prefix}_spend")}',
        f'订单 {row.get(f"{prefix}_ad_orders")}',
        f'本 SKU 单 {row.get(f"{prefix}_promoted_ad_orders")}',
        f'光环单 {row.get(f"{prefix}_halo_ad_orders")}',
        f'ACOS {row.get(f"{prefix}_acos") or "N/A"}',
    ]
    metrics = [metric for metric in metrics if not metric.endswith(" None") and not metric.endswith(" ")]
...
def _render_keyword_action_effect_review_rows(rows: list[dict[str, str]], limit: int = 5) -> str:
    ...
    for row in sorted_rows:
        judgement = str(row.get("judgement") or "数据不足")
        tag = "tag-green" if judgement in {"初步有效", "有改善迹象", "有效"} else ("tag-yellow" if judgement in {"样本不足", "待7天确认", "待观察", "数据不足"} else "tag-red")
        title = f"{row.get('marketplace') or ''}｜{row.get('search_term_or_target') or ''}"
        days = str(row.get("days_since_execution") or "").strip()
